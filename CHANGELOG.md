...
- In the Minimal view, collapsing a running turn's expanded details no longer fights itself: the fold grows the padding back to the viewport height (so the list bottom never rises into the viewport and clamps the scroll), then the viewport glides once to the pinned position — the working header no longer gets dragged down by the shrinking details and snapped back on re-pin.
