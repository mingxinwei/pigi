...
- When a turn in the Minimal view finishes, its final summary now streams in word by word — like a normal message being produced at the bottom of the list — instead of appearing all at once: the working-area pin is released the moment the turn ends, the view glides to the bottom and follows the streaming summary (scrolling away locks the position and stops the follow, exactly like any other output), and the list settles at the bottom when the stream completes.
